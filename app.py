...
def receive_data():
    ...
    sem_no = sem_no_pattern.group(
        1).strip() if sem_no_pattern else ''
    ...
    table_data = [{'Subject_Name': match[0], 'Subject_Code': match[1],
                   ...
                   'Total_Marks': str(int(match[2]) + int(match[3])), 'Grade': match[4], 'Final_Marks': int(match[2]) + int(match[3]), 'Points': gradeMapping.get(match[4], 0)} for match in matches]
